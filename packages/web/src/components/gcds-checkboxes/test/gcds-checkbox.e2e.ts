...
describe('gcds-checkboxes a11y tests', () => {
  ...
  it('aria-invalid', async () => {
    ...
    await page.setContent(
      '<gcds-checkboxes label="Label" checkbox-id="aria-invalid" error-message="Has error" />',
    );
  ...
  it('colour contrast', async () => {
    const page = await newE2EPage();
    await page.setContent(`
      <gcds-checkboxes label="Label" checkbox-id="colour-contrast" />
    `);
  ...
  it('colour contrast - error', async () => {
    const page = await newE2EPage();
    await page.setContent(`
      <gcds-checkboxes label="Label" checkbox-id="colour-contrast" has-error />
    `);
  ...
  it('colour contrast - disabled', async () => {
    const page = await newE2EPage();
    await page.setContent(`
      <gcds-checkboxes label="Label" checkbox-id="colour-contrast" disabled />
    `);
  ...
  it('checkbox keyboard focus', async () => {
    const page = await newE2EPage();
    await page.setContent(`
      <gcds-checkboxes label="Label" checkbox-id="keyboard-focus" />
    `);
  ...
  it('checkbox contains label', async () => {
    ...
    await page.setContent(
      '<gcds-checkboxes label="Label" checkbox-id="contains-label" />',
    );
