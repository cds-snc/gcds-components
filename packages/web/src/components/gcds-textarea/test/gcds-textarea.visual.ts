...
test.describe('gcds-textarea', () => {
  ...
  test('maxlength', async ({ page }) => {
    await expect(
      page.locator('[data-variant="maxlength"] .preview-component'),
    ).toHaveScreenshot('maxlength.png');
  });
  ...
  test('rows', async ({ page }) => {
    await expect(
      page.locator('[data-variant="rows"] .preview-component'),
    ).toHaveScreenshot('rows.png');
  });
