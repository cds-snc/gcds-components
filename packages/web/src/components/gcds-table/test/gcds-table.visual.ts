...
test.describe('gcds-table', () => {
  ...
  test('filter', async ({ page }) => {
    await expect(
      page.locator('[data-variant="filter"] .preview-component'),
    ).toHaveScreenshot('filter.png');
  });

  test('pagination', async ({ page }) => {
    await expect(
      page.locator('[data-variant="pagination"] .preview-component'),
    ).toHaveScreenshot('pagination.png');
  });
