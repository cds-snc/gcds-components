...
export async function getNavItems(el) {
  ...
  /**
* Create mobile nav element in passed nav
* @param {HTMLElement} nav
*/
export async function configureMobileNav(nav: HTMLElement) {
  let children = Array.from(nav.children);

  let navGroup = document.createElement("gcds-nav-group") as HTMLGcdsNavGroupElement;
  navGroup.heading = "Menu";
  navGroup.classList.add('gcds-mobile-nav');

  for (let element of children) {
    if (!element.hasAttribute("slot")) {
      navGroup.appendChild(element);
    }
  }
...
/**
* Unpack nav items from mobile nav in passed nav
* @param {HTMLElement} nav
*/
export async function unpackMobileNav(nav: HTMLElement) {
  let mobileNavChildren = [];
  let mobileNav: Element;

  for (let item of Array.from(nav.children)) {
    if (!item.hasAttribute("slot")) {
      mobileNavChildren = Array.from(item.children);
      mobileNav = item;
    }
  }

  for (let element of mobileNavChildren) {
